...
**CRITICAL RULE:** This package is the center of the Hexagonal Architecture. It must **NEVER** import from `@repo/adapter-drizzle`, `@repo/adapter-mongo`, `express`, or any database/network libraries. Dependencies must only point inward.
...
- **Behavior:** 1. Captures `performance.now()` start time. 2. Parses the AST using `@repo/graphql-utils`. 3. Builds the strict `DataQueryPlan<ArticleEntity>`. 4. Selects the injected repository from the `context` based on the `dbType` argument. 5. Executes the query and calculates `latencyMs` and `payloadSizeKb`.
