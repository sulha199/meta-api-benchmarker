# Architecture: @repo/schema-drizzle

## 1. Package Purpose
The `schema-drizzle` package acts as the **PostgreSQL Infrastructure Adapter**. It implements the generic database interfaces defined in `@repo/db-core` using the Drizzle ORM. 
...
* **Path:** `packages/schema-drizzle/src/adapters/DrizzleAdapter.ts`
...
* **Path:** `packages/schema-drizzle/src/schema/ast.ts` (for Articles/Comments) and `schema/shared.ts` (for Visitors/Logs).
...
* **Path:** `packages/schema-drizzle/src/repositories/DrizzleArticleRepository.ts`
...
When modifying or interacting with `@repo/schema-drizzle`, the AI must follow these rules:
