...
1. **Consult the Map First:** Before writing _any_ code or suggesting a refactor, you must cross-reference the specific package's architecture document.
...
6. **Imports & Module Resolution:** Never use `.js` or `.ts` suffixes in import paths. Always rely on `"moduleResolution": "bundler"` allowing for clean, extensionless imports.
7. **Environment Variables:** All backend configurations run off a single, centralized `.env` file at the root of the monorepo. Never create localized `.env` files in sub-packages. When initializing `dotenv`, always load from the root explicitly: `config({ path: resolve(process.cwd(), "../../.env") })` or `resolve(__dirname, "../../.env")`. Assume `process.env` is globally typed via `@repo/shared-backend`.
