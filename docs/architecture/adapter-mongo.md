# Architecture: @repo/adapter-mongo

## 1. Package Purpose

The `adapter-mongo` package acts as the **MongoDB Infrastructure Adapter**. It implements the generic database interfaces defined in `@repo/db-core` using Mongoose.
...
- **Path:** `packages/adapter-mongo/src/adapters/MongoAdapter.ts`
...
- **Path:** `packages/adapter-mongo/src/schema/models.ts`
...
- **Path:** `packages/adapter-mongo/src/repositories/MongoArticleRepository.ts`
- **The Data Mapper:** Maps the raw Mongoose object to the pure Domain Entity.
  - _Crucial Translation:_ It handles the normalization of MongoDB's `_id` (ObjectId) to the domain's standard `id` (string).

## 3. AI Agent Instructions

When modifying or interacting with `@repo/adapter-mongo`, the AI must follow these rules:
