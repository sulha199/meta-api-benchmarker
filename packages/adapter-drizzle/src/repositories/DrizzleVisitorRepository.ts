  ...
  implements IVisitorRepository
{
  ...
  protected toDomain(
    dbRecord: DbVisitorSelect & { logs: VisitLogEntity[] },
  ): VisitorEntity {
    return {
      id: dbRecord.id,
  ...
  protected toAdapterQuery(
    query: QueryCriteria<VisitorEntity>,
  ): QueryCriteria<DbVisitorSelect> {
    const { where, orderBy, ...rest } = query;
    return {
      ...rest,
      where: where ? this.toPersistence(where) : undefined,
      orderBy,
    };
  }
